...
| **`ORDER BY` and keyed `GROUP BY` still buffer** | Only `DISTINCT` spills. The other two hold every row in memory, so a large one is refused by `--max-blocking-rows` rather than answered | Make the pattern more selective. A `LIMIT` does not help an `ORDER BY`: the k smallest cannot be known without sorting all of them |
