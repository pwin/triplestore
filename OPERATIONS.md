...
| **`ORDER BY` without a `LIMIT`, and keyed `GROUP BY`, still buffer** | Only `DISTINCT` spills. `SELECT … ORDER BY … LIMIT` is answered from a heap of the rows it returns, whatever its input; a sort in any other shape — no `LIMIT`, or a `DISTINCT` between the sort and the slice — and a keyed `GROUP BY` hold every row in memory, so a large one is refused by `--max-blocking-rows` rather than answered | Make the pattern more selective, or give the sort a `LIMIT` |
