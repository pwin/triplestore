...
| **`DISTINCT` comes back sorted** | `--spill-distinct` (default 128 MiB) answers `SELECT DISTINCT` and `COUNT(DISTINCT *)` by sorting and spilling rather than from a hash set, which is what lets one larger than memory finish. Sorted output is the consequence | SPARQL promises no order for `DISTINCT`, so this is conformant. If something downstream depends on arrival order, `--spill-distinct 0` restores the old behaviour — and with it the failure mode |
